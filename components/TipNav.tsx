...
export type NavItem = { id: string; label: string; partnerName?: string; locked?: boolean };
...
// Locked tips (behind the gate) are shown greyed with a lock and jump to the form.
export function TipRail({ items, activeId, accent, jumpTo, gateId, headingLabel }: { items: NavItem[]; activeId: string; accent: string; jumpTo: (id: string) => void; gateId: string; headingLabel: string }) {
  let lastPartner = "";
  ...
  return (
    <nav aria-label={headingLabel} style={{ position: "sticky", top: HEADER_OFFSET + 8, alignSelf: "start" }}>
      ...
      <ol style={{ listStyle: "none", margin: 0, padding: 0 }}>
        {items.map((it, i) => {
          const on = it.id === activeId && !it.locked;
          ...
          return (
            <li key={it.id}>
              ...
              <a
                href={it.locked ? `#${gateId}` : `#${it.id}`}
                aria-current={on ? "true" : undefined}
                title={it.locked ? headingLabel : undefined}
                onClick={(e) => {
                  e.preventDefault();
                  jumpTo(it.locked ? gateId : it.id);
                }}
                style={{ display: "flex", gap: 10, alignItems: "baseline", padding: "7px 10px", borderLeft: `2px solid ${on ? accent : P.p200}`, color: it.locked ? P.p400 : on ? accent : P.p700, fontFamily: BODY, fontSize: 13, fontWeight: on ? 600 : 500, textDecoration: "none", lineHeight: 1.35 }}
              >
                <span style={{ fontFamily: DISP, fontWeight: 700, fontSize: 11, minWidth: 16, color: it.locked ? P.p300 : on ? accent : P.p400 }}>{String(i + 1).padStart(2, "0")}</span>
                <span style={{ flex: 1 }}>{it.label}</span>
                {it.locked ? <Icon name="lock" color={P.p400} size={12} /> : null}
              </a>
            </li>
          );
        })}
...
// expands the tip list to jump. Locked tips are greyed and jump to the form.
export function TipMobileNav({ items, activeId, accent, jumpTo, gateId, headingLabel }: { items: NavItem[]; activeId: string; accent: string; jumpTo: (id: string) => void; gateId: string; headingLabel: string }) {
  const [open, setOpen] = useState(false);
  const activeIndex = Math.max(0, items.findIndex((it) => it.id === activeId));
  const cur = items[activeIndex];
  const pct = items.length > 0 ? Math.round(((activeIndex + 1) / items.length) * 100) : 0;
  return (
    <div style={{ position: "sticky", top: 64, zIndex: 16, marginBottom: 16 }}>
      <div role="progressbar" aria-valuemin={0} aria-valuemax={items.length} aria-valuenow={activeIndex + 1} style={{ height: 3, background: P.p200, borderRadius: 999, marginBottom: 8 }}>
        <div style={{ height: "100%", width: `${pct}%`, background: accent, borderRadius: 999, transition: "width .35s cubic-bezier(.2,.8,.2,1)" }} />
      </div>
      <button
        type="button"
          ...
          {items.map((it, i) => {
            ...
            return (
              <a
                ...
                style={{ display: "flex", gap: 10, alignItems: "center", minHeight: 44, padding: "0 14px", borderTop: i > 0 ? `1px solid ${P.p100}` : "none", color: it.locked ? P.p400 : on ? accent : P.p900, fontFamily: BODY, fontSize: 14, fontWeight: on ? 600 : 500, textDecoration: "none" }}
              >
                <span style={{ fontFamily: DISP, fontWeight: 700, fontSize: 12, color: it.locked ? P.p300 : accent, minWidth: 18 }}>{String(i + 1).padStart(2, "0")}</span>
                <span style={{ flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{it.label}</span>
                {it.locked ? <Icon name="lock" color={P.p400} size={13} /> : null}
              </a>
            );
          })}
        </div>
